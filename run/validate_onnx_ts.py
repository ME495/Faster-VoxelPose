...
def main():
    ...
    with torch.no_grad():
        ...
        backbone_inference_times = np.array(backbone_inference_times)
        model_inference_times = np.array(model_inference_times)

        mean_inference_time = np.mean(inference_times)
        mean_backbone_time = np.mean(backbone_inference_times)
        mean_model_time = np.mean(model_inference_times)
        ...
        if len(inference_times) > 10: # Consider making this warmup count configurable
            ...
            backbone_inference_times_no_first = backbone_inference_times[10:]
            model_inference_times_no_first = model_inference_times[10:]

            mean_inference_time_no_first = np.mean(inference_times_no_first)
            mean_backbone_time_no_first = np.mean(backbone_inference_times_no_first)
            mean_model_time_no_first = np.mean(model_inference_times_no_first)
            mean_fps_no_first = 1.0 / mean_inference_time_no_first
            logger.info(f'平均总推理时间(忽略前10次): {mean_inference_time_no_first:.4f}秒, FPS: {mean_fps_no_first:.2f}')
            logger.info(f'  平均Backbone推理时间(忽略前10次): {mean_backbone_time_no_first:.4f}秒')
            logger.info(f'  平均Model推理时间(忽略前10次): {mean_model_time_no_first:.4f}秒')
        
        logger.info(f'平均总推理时间: {mean_inference_time:.4f}秒, FPS: {mean_fps:.2f}')
        logger.info(f'  平均Backbone推理时间: {mean_backbone_time:.4f}秒')
        logger.info(f'  平均Model推理时间: {mean_model_time:.4f}秒')
        logger.info(f'最快总推理时间: {np.min(inference_times):.4f}秒, 最慢: {np.max(inference_times):.4f}秒')
