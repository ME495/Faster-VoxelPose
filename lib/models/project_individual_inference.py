...
class ProjectLayerInference(nn.Module):
    ...
    def forward(self, heatmaps, index, proposal_centers, sample_grid_fine):
        """
        前向传播函数：构建个体特征体素
        ...
        for i in range(num_people):
            ...
            if torch.sum(start[i] >= end[i]) > 0:
                continue
            # 提取当前人体有效范围内的采样网格
            sample_grid = sample_grid_fine[:, start[i, 0]:end[i, 0], start[i, 1]:end[i, 1], start[i, 2]:end[i, 2]].reshape(n, 1, -1, 2)
